...
Value for the `clOrdId` is assigned by the trader. It should be a unique string for each order. This string can be any length, but only the first 16 bytes used by the exchange. If the length be less than 16 bytes the trading engine will add zero bytes to make it 16 bytes total.
...
For `BTCUSD-PERP`: order price should be positive and <u>multiple of 5</u>, order quantity positive and <u>integral</u>.
...
The server will respond with appropriate `status` value.

If provided token isn't valid you will receive the following message:
...
You need to send `auth` message with your token every time you get this error.
...
The order can be accepted or rejected by the exchange.

If the order has been accepted by the trading engine you will receive the following message:
...
The value of `clOrdId` could be used to cancel this order in the future (in case of `LIMIT` order).

------

#### Order Fills

When order is filled you will receive the following message:
...
The field `contracts` contains a set of contacts that were added to your position. 

You can use the value of `contractId` to close specific contract in the future by this ID.

Note: the value of `newClOrdId` is generated by the exchange. It's the ID of `FILLED` order which has the reference to the originally placed order `origClOrdId`.

------

#### Order Cancellation

Specific order could be cancelled by its `clOrdId` via the following message:
...
More than one order could be cancelled using the following message:
...
Field `orders` contains all the orders that were cancelled. 
...
The field `orderIds` contains an array of `clOrdId`s which were created by the exchange to close the contract. Trader also will receive `orderStatus` and `orderFilled` messages related to these orders. The latter one will contain the information about affected contracts.

Note: if trader closes only a part of the contract the exchange will generate new ID for remained part of the contract (this ID could be found in `orderFilled` message).
...
A sequence of messages that will be received as a result is the same as in case of closing a single contract (`contractClosed`, `orderStatus`, `orderFilled`).
